...
class TokensService {
	...
	async createRefreshToken(userId) {
		const refreshToken = jwt.sign({ id: userId }, process.env.JWT_REFRESH_SIGNATURE);
	...
	refreshTokens(refreshToken) {
		try {
			const { id } = jwt.verify(refreshToken, process.env.JWT_REFRESH_SIGNATURE);
	...
	getTokenFromRequest(req) {
		...
		return authHeader.split(' ')[0] === 'Bearer' && authHeader.split(' ')[1];
	}
